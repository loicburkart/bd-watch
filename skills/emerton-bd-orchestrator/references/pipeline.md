...
| **02 Qualify** | `Trigger[]` | `QualifiedTrigger[]` (P1–P3) | `steps/step02_qualify.py` + `targeting_matrix.json` — packaged in `emerton-signal-watch` | ✅ Matrix scoring live (sector × geo × function, worst-axis + P1-sector bypass). |
...
| **CRM ingestion** | HubSpot (live) | reminders / post-mortem `.xlsx` + never-won prospect list | packaged routines `crm-reminders-routine`, `crm-post-mortem-routine`, `crm-prospects-for-news-screening` (+ `src/bd_watch/feeders.py`) | ✅ Packaged (Ugo). Generate the spreadsheet on demand, then hand to the drafter. |
...
- **Watch + Qualify:** use the `emerton-signal-watch` skill (or `python triggers_module/nominations_scraper.py`).
- **Generate CRM spreadsheets from HubSpot:** the `crm-reminders-routine`, `crm-post-mortem-routine`, and
  `crm-prospects-for-news-screening` skills.
...
- **Discovery path** (Watch → Qualify → Contact → Draft → Review): **runs end-to-end on `main`**. Watch +
  Qualify are packaged (`emerton-signal-watch`, matrix scoring live). **One known gap to flag in the run
  report: contact resolution is mocked** — `step03_contact.identify_contact()` returns a placeholder
  profile; the real `src/bd_watch/identify_contact/` module (Lusha + web + LLM) is **not yet wired into the
  hook**. Treat identified people as `contact_unverified` until that wiring lands.
- The orchestrator never fabricates a trigger, contact, or fact to fill a gap — it marks them
  `pending`/`unverified` and proceeds.

When step03 is wired to `identify_contact`, **no orchestrator change is needed**: it already calls each
stage in order through the shared schemas.
